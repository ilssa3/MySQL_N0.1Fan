SET sql_mode = '';
CREATE DATABASE IF NOT EXISTS `softuni`;
USE `softuni`;

create table towns(
town_id int(10)auto_increment not null primary key,
name varchar(50)
);
...
`name` varchar(50), 
`manager_id` int(10) references employees(employee_id)
);
...
`employee_id` int(10) not null primary key,
 ...
 `department_id` int (10)references departments(department_id),
 `manager_id` int(10) references  employees (employee_id), 
 `hire_date` timestamp(6), 
 `salary` decimal(19,4),
 `address_id` int(10)references addresses(address_id)
);
...
employee_id int(10) references employees (employee_id),
project_id int(10) references projects(project_id)
);
